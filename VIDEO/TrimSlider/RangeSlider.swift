...
class RangeSliderTrackLayer: CALayer {
    ...
    override func draw(in ctx: CGContext) {
        ...
        ctx.setFillColor(slider.trimTintColor.cgColor)
        let end1 = CGFloat(slider.positionForValue(slider.lowerValue))
        let rect1 = CGRect(x: 0.0, y : 0.0, width: end1, height: bounds.height)
        ctx.fill(rect1)
        
        ctx.setFillColor(slider.trimTintColor.cgColor)
        let start2 = CGFloat(slider.positionForValue(slider.upperValue))
        let end2 = bounds.width
        let rect2 = CGRect(x:start2, y:0.0, width: end2-start2, height: bounds.height)
        ctx.fill(rect2)
    }
}
