...
- (void)setThemeColor:(UIColor *)themeColor {
    ...
    self.leftThumbImage = [UIImage imageNamed:@"lower_image_bar_tool"];
    self.rightThumbImage = [UIImage imageNamed:@"lower_image_bar_tool2"];
...
//        [self.contentView addSubview:rulerView];
    ...
    self.leftOverlayView = [[HitTestView alloc] initWithFrame:CGRectMake(0, 0, self.scrollView.contentSize.width, CGRectGetHeight(self.frameView.frame))];
    self.leftOverlayView.hitTestEdgeInsets = UIEdgeInsetsMake(0, 0, 0, -(EDGE_EXTENSION_FOR_THUMB));
    CGRect leftThumbFrame = CGRectMake(CGRectGetMaxX(self.leftOverlayView.bounds)-self.thumbWidth, 0, self.thumbWidth, CGRectGetHeight(self.frameView.frame));
    ...
    self.rightOverlayView = [[HitTestView alloc] initWithFrame:CGRectMake(rightViewFrameX+100, 0, self.overlayWidth, CGRectGetHeight(self.frameView.frame))];
    ...
    self.rightOverlayView.hidden = true;
    self.leftThumbView.hidden = true;
...
- (void)moveOverlayAccordingToPoint:(CGPoint *)point {
...
- (void)moveOverlayView:(UIPanGestureRecognizer *)gesture
{
    ...
    switch (gesture.state) {
        ...
        case UIGestureRecognizerStateChanged:
        {
            ...
            //------------------------------------------------------------------------------------------------------------
            // Right
            [self moveOverlayAccordingToPoint:&point];
            //------------------------------------------------------------------------------------------------------------
...
- (void)notifyDelegateOfDidChange
{
    ...
    NSLog(@"start:%f , end:%f contentOffset.x:%f", start , end ,self.scrollView.contentOffset.x);
...
//    NSLog(@"%f, %f, %f", scrollView.contentOffset.x , currentPosition, self.rightOverlayView.frame.origin.x);
    NSMutableDictionary *dict = [[NSMutableDictionary alloc] init];
    [dict setValue:[NSString stringWithFormat:@"%f",self.scrollView.contentOffset.x] forKey:@"contentOffset"];
    [dict setValue:[NSString stringWithFormat:@"%f",self.rightOverlayView.frame.origin.x] forKey:@"rightOverlayViewOrigin"];
...
//        if ([self.delegate respondsToSelector:@selector(disableSeekPosControl:)]) {
//            [self.delegate disableSeekPosControl:@"start"];
//        }
//    }else if (scrollView.contentOffset.x+CGRectGetWidth([UIScreen mainScreen].bounds)/2 <= self.leftOverlayView.frame.origin.y){
//        if ([self.delegate respondsToSelector:@selector(disableSeekPosControl:)]) {
//            [self.delegate disableSeekPosControl:@"end"];
//        }
//
//    }
}
...
-(void)setPointStart{
    ...
    CGRect rect = CGRectMake(0, 0, point, CGRectGetHeight(self.frameView.frame));
    self.leftOverlayView.frame = rect;
    CGRect leftThumbFrame = CGRectMake(CGRectGetMaxX(self.leftOverlayView.bounds)-self.thumbWidth, 0, self.thumbWidth, CGRectGetHeight(self.frameView.frame));
    self.leftThumbView.frame = leftThumbFrame;
    if (self.rightOverlayView.isHidden) {
        self.leftThumbView.hidden = false;
